    ...
    def _register_definitions(self):
        for definition in (
            ...
            AIAgentInterfaceDefinition(
                key="jobs/history",
                method="GET",
                description="Query persisted job history with fields similar to job details; completed records can retain used_memory_gb maximum memory and used_cpu_cores_avg average CPU cores used.",
                arguments_description="Optional: job_id, user, account, partition, qos, state, keyword, start, end, sort, order, page, page_size, limit. Use this when live job data is missing or a completed job needs historical resource usage.",
                write=False,
            ...
            AIAgentInterfaceDefinition(
                ...
                method="GET",
                description="Get one persisted job history record by database record_id, including job-like detail fields and completed-job usage fields when available.",
                arguments_description="Required: record_id. Returned completed records may include used_memory_gb maximum memory and used_cpu_cores_avg average CPU cores used.",
                write=False,
