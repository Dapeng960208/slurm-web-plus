...
- 后台用户工具日聚合按 `[user_metrics].aggregation_interval` 周期更新当天 UTC 自然日统计，并与 `slurmweb/repair-user-tool-daily-stats.py` 维护脚本复用同一套聚合函数，保持工具归类、空值过滤和插入口径一致
...
- 用户分析资源统计明确按已完成作业计算：
  ...
  - 平均 CPU 核数：当前日聚合只使用 `used_cpu_cores_avg > 0`，无有效样本时保存 `0`
