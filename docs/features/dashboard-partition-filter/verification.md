...
- Agent `stats` 已支持读取 `partition` query，并用队列结果重新聚合 jobs/resources。
...
- 若其他 Agent 或后续提交把 `metrics_db.request` 正式升级为支持 `partition`，应再补一层更靠近 metrics DB 的测试，验证不仅“参数传到了”，还要验证“结果确实按队列过滤了”。
