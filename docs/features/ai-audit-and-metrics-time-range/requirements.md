...
- 分析页新增平均排队时长曲线，统一按秒展示，数据来自历史已完成作业的 `submit_time -> start_time` 差值聚合。
- 平均排队时长曲线只使用卡片右上角自身的时间范围，不再在页面页头右侧额外展示全局时间组件。
- 平均排队时长曲线支持 `minute / hour / day` 聚合粒度切换；这里的粒度表示时间桶大小，图中数值始终为秒。默认最近 1 小时按分钟聚合，小时按小时聚合，天按天聚合。
- 图表横轴固定使用卡片当前请求历史作业时的 `start` / `end` 窗口，即使窗口内只有一个有效样本，也不会把 X 轴压缩到单个毫秒点附近。
- 平均排队时长曲线在切换时间窗口并重新请求 `jobs/history` 时必须显示图表加载骨架，加载期间不展示旧窗口曲线。
- 平均排队时长曲线在等待时间超过 `60` 秒后，线条与填充颜色按等待时长从主题淡绿色连续过渡到橙色、红色，用于强化高等待区间的风险感知。
...
- 对话页三栏高度链路需固定：
  - 左侧会话历史独立滚动。
  - 中间聊天列固定为“消息滚动区 + 底部输入区”。
  - 右侧执行 trace 独立滚动。
- 对话页所在的 `ui-content-scroll` 本身必须锁定在浏览器可视区内；聊天列在该可视区内占满剩余高度，composer 贴浏览器可视区底部，而不是贴整页内容底部。
- 中间消息区内容增长时只在消息区内部滚动，不得继续把聊天列整体撑高或把输入区向下挤出可视区。
- 输入区始终 `shrink-0` 固定在左侧聊天列底部；空态快捷问题按钮仍保留在消息区内部。
...
- 节点热点持久化使用数据库表 `node_metric_samples`：
  ...
  - Agent `/info` 的 `capabilities.node_hotspots` 表示该链路是否可用，前端必须按该能力决定是否请求 `analysis/node-hotspots`
...
- 数据库不可用或 node metrics 不可用时，节点热点持久化不会启用；此时 `analysis/node-hotspots` 接口不可用，`Cluster Analysis` 前端不应主动请求该接口。
