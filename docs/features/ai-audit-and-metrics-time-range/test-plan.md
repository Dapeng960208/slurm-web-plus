...
- 刷新页面后保留已应用的起止时间。
- 用户分析摘要卡显示运行、排队、失败、取消状态作业数。

### 2.2.1 集群分析时间窗与热点

- 集群分析平均排队时长卡片使用自身的时间范围按钮与弹框；页面页头右侧不再展示额外全局时间组件。
...
- 应用自定义窗口后：
  ...
  都应跟随新的 `start` / `end`。
- 页面展示平均排队时长曲线，单位为秒，口径为 `submit_time -> start_time`。
- 平均排队时长曲线支持分钟、小时、天三种聚合粒度切换；默认最近 1 小时按分钟桶聚合，切换聚合粒度不改变秒级展示单位。
- 平均排队时长曲线横轴按卡片时间范围展开；单个有效样本时仍显示完整窗口，而不是毫秒级单点窗口。
- 切换平均排队时长曲线时间窗口后，在新的 `jobs/history` 响应返回前展示图表加载骨架，旧曲线不再继续显示。
- 平均排队时长曲线在等待时间不超过 `60` 秒时保持主题淡绿色，超过后会随等待时长逐步过渡到橙色和红色。
...
- 左侧会话历史、中间消息区、右侧 trace 各自保持独立滚动。
- 中间消息区保持固定高度滚动容器，composer 与 message scroller 同属左侧聊天列，且 composer 不在消息滚动区内部。
- composer 保持 `shrink-0`，长对话或流式输出时不会被消息区继续向下挤走。
- 对话页外层主内容容器具备固定可视区高度链路；composer 最终贴的是浏览器可视区底部，而不是整页文档流底部。
- 用户消息与 AI 回复均展示复制按钮。
- 点击复制按钮时写入对应消息原文。
