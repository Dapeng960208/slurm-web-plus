...
- AI planner system message 会注入当前交互上下文，至少包含当前 `user.login` 和 cluster；第一人称问题默认指向该当前用户
...
- 可按标题过滤审计摘要
- 审计列表不再在同页右侧内嵌详情
- 可打开独立会话详情页查看消息与工具调用记录
- 已删除会话显示 `deleted_at` 与 `deleted_by`

新增前端路由：

- `/:cluster/admin/ai/conversations/:conversation_id`
